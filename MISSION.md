...
1. `npm run build`, `npm run typecheck`, `npm run lint`, `npm test -- --run`, and `npm run test:e2e` all exit 0.
...
6. Core content supports route, POI and shape layers with create/select/edit/hide/lock/reorder/delete and undo/redo.
...
10. Export tests assert dimensions and named SVG groups; E2E tests exercise the complete nominal flow in Chromium, Firefox and WebKit.
11. A 390px smoke test has no body horizontal overflow, and the browser console is clean in nominal desktop/mobile flows.
12. `docs/FINDINGS.md` records evidence, open limitations and exact verification output. `docs/COMPLETE.md` is created only after every gate passes and includes commands, output summaries and the final commit SHA.
...
5. Implement the smallest complete slice, then run focused and full relevant tests.
6. Verify changed UI in a real browser screenshot at desktop and relevant responsive widths.
7. Update `docs/FINDINGS.md` with evidence and unresolved risks.
8. Run independent code review for multi-file changes, fix blockers, and commit verified progress.
9. Report concise progress and the next unresolved slice.
