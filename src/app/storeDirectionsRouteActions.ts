...
import { isRouteAuthoringOptions, type RouteAuthoringOptions } from '../domain/routeProfiles';
import type { ProjectState } from './store';
import { commitDocument, replaceLayers, type ProjectSet } from './storeDocument';

type CanonicalDirectionsRoute = DirectionsRouteInput & { options: RouteAuthoringOptions };
type PositionLimits = { maximum: number; minimum: number; requireAllDistinct: boolean };

const PROFILE_BY_MARKER = { car: 'driving', walk: 'walking', bike: 'cycling' } as const;
...
function canonicalPositions(value: unknown, limits: PositionLimits): [number, number][] | null {
  if (!Array.isArray(value) || value.length < limits.minimum || value.length > limits.maximum) return null;
  ...
  const distinct = new Set(positions.map((position) => `${position[0]},${position[1]}`)).size;
  if (distinct < 2 || (limits.requireAllDistinct && distinct !== positions.length)) return null;
  return positions;
}
...
function canonicalDirectionsRoute(
  ...
  if (!isRouteAuthoringOptions(options) || options.lineShape !== 'road') return null;
  const input = directionsInput(value);
  if (!input) return null;
  const waypoints = canonicalPositions(input.waypoints, {
    minimum: 2, maximum: 25, requireAllDistinct: true,
  });
  const geometry = canonicalPositions(input.geometry, {
    minimum: 2, maximum: 50_000, requireAllDistinct: false,
  });
  if (!waypoints || !geometry) return null;
  const expectedProfile = PROFILE_BY_MARKER[options.travelProfile as keyof typeof PROFILE_BY_MARKER];
  if (input.profile !== expectedProfile) return null;
  if (!isNonNegativeFinite(input.distanceMeters) || !isNonNegativeFinite(input.durationSeconds)) return null;
  return {
    geometry,
    waypoints,
...
function routeLayer(
  ...
  return {
    ...
    appearance: {
      ...appearance,
      travelProfile: input.options.travelProfile,
      showTravelModeIcon: input.options.showTravelModeIcon,
    },
...
export function createDirectionsRouteAction(set: ProjectSet): ProjectState['createDirectionsRoute'] {
  return (candidate, options, expectedDocumentEpoch) => {
