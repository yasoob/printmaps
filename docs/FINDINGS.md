...
Add a bounded, pasteable coordinate-only POI spreadsheet (name, longitude and latitude, 300-row cap) as one undoable batch, while keeping address geocoding fail-closed under the accepted provider-storage boundary. Validated custom image-marker assets follow as the next POI sub-slice.
