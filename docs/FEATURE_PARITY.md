...
| POI placement, coordinates, appearance | Partial | Point authoring and coordinate controls in `src/app/`; three canonical marker shapes, six semantic symbols and optional 40-character labels in `src/domain/poiMarkers.ts`; live MapLibre plus vector SVG/PDF coverage in `tests/e2e/layer-appearance.spec.ts` and focused unit suites | The semantic symbols are compact letter badges rather than a full pictographic catalogue. Custom image upload, searchable placement and spreadsheet/batch entry remain absent. PDF labels currently fail closed outside the font's supported Latin-1 range rather than silently corrupting text. |
...
1. Bounded coordinate spreadsheet/batch POI entry and validated custom image markers.
