...
| POI placement, coordinates, appearance and batch entry | Partial | Point authoring and coordinate controls in `src/app/`; three canonical marker shapes, six semantic symbols and optional 40-character labels in `src/domain/poiMarkers.ts`; bounded tab-separated Name/Longitude/Latitude batches under `src/domain/poiSpreadsheet.ts` and `tests/e2e/poi-authoring.spec.ts`; live MapLibre plus vector SVG/PDF coverage in `tests/e2e/layer-appearance.spec.ts` | Coordinate-only spreadsheet paste now creates up to 300 named/labelled POIs as one Undo/Redo transaction. Address columns/geocoding remain fail-closed under the provider-storage decision. Custom image upload and searchable placement remain absent; semantic symbols are compact badges rather than a full pictographic catalogue. PDF labels fail closed outside Latin-1 rather than silently corrupting text. |
...
1. Validated custom image markers.
