...
test('Danish region catalogue creates a durable area with print parity', async ({ page }, testInfo) => {
  ...
  await page.getByRole('combobox', { name: 'Region country' }).selectOption('DNK');
  const regions = page.getByRole('group', { name: 'Denmark regions' });
  await expect(regions.getByRole('checkbox')).toHaveCount(5);
  await regions.getByRole('checkbox', { name: 'Capital Region' }).check();
  await page.getByRole('button', { name: 'Add selected area' }).click();
  await expect(page.getByRole('button', { name: 'Select Capital Region' })).toHaveAttribute('aria-current', 'true');
  await expect(page.getByTestId('map-canvas')).toHaveAttribute('data-map-layer-geometry', /admin-dk-84:/);
  ...
  const region = savedProject.layers.find(({ id }: { id: string }) => id === 'admin-dk-84');
  expect(savedProject.schemaVersion).toBe(21);
  expect(region).toMatchObject({ name: 'Capital Region', geometry: { type: 'MultiPolygon' } });
  ...
  const svgPath = testInfo.outputPath('denmark-region.layered.svg');
  await svgDownload.saveAs(svgPath);
  const svg = await readFile(svgPath, 'utf8');
  expect(svg).toContain('data-layer-name="Capital Region"');
